...
export class BrowserPool {
  ...
  async getBrowser(): Promise<Browser> {
    ...
    if (this.browsers.has(browserType)) {
      ...
      // Browser disconnected — evict and fall through to launch a new one
      console.error(`[BrowserPool] Browser ${browserType} disconnected, evicting`);
      this.browsers.delete(browserType);
      try {
        await browser.close();
      } catch (closeError) {
        console.error(`[BrowserPool] Error closing disconnected browser:`, closeError);
      }
    }
